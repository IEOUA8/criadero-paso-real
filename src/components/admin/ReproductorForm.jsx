...
const ReproductorForm = ({ reproductor, onClose, onSaved }) => {
  ...
  useEffect(() => {
    if (reproductor) {
      setFormData({
        ...
        raza: reproductor.raza || 'Burro Criollo Colombiano de Silla',
        sexo: reproductor.sexo || 'Macho',
        edad: reproductor.edad || '',
        color: reproductor.color || '',
        alzada: reproductor.alzada || '',
        criador: reproductor.criador || '',
        propietario: reproductor.propietario || 'Criadero Paso Real',
        padre: reproductor.padre || '',
        madre: reproductor.madre || '',
        linea_genetica: reproductor.linea_genetica || '',
        descripcion: reproductor.descripcion || '',
        fotos: reproductor.fotos || []
      });
    }
  }, [reproductor]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast({
        title: "Éxito",
        description: `Reproductor ${reproductor ? 'actualizado' : 'creado'} correctamente.`,
      });
      onSaved();
    } catch (err) {
      console.error(err);
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl my-8">
        <div className="flex justify-between items-center p-6 border-b border-gray-100">
          <h2 className="font-playfair text-2xl font-bold text-[#0B0B0B]">
            {reproductor ? 'Editar Reproductor' : 'Nuevo Reproductor'}
          </h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <Label>Padre</Label>
              <Input name="padre" value={formData.padre} onChange={handleChange} className="text-gray-900" />
            </div>
            <div className="space-y-2">
              <Label>Madre</Label>
              <Input name="madre" value={formData.madre} onChange={handleChange} className="text-gray-900" />
            </div>
          </div>

          <div className="space-y-2">
            <Label>Línea genética</Label>
            <Textarea name="linea_genetica" value={formData.linea_genetica} onChange={handleChange} rows={3} className="text-gray-900" />
          </div>
          ...
          {/* Simple Image URL array input for now since Storage bucket might require separate setup/policies */}
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-gray-500">Introduce URLs válidas de imágenes para el catálogo.</p>
          </div>
        ...
        <div className="flex justify-end gap-4 p-6 border-t border-gray-100 bg-gray-50 rounded-b-xl">
          ...
          <Button onClick={handleSubmit} disabled={loading} className="bg-[#C8A94B] hover:bg-[#C8A94B]/90 text-[#0B0B0B] font-bold">
            ...
            {reproductor ? 'Actualizar' : 'Guardar'}
          </Button>
        </div>
      </div>
    </div>
...
export default ReproductorForm;