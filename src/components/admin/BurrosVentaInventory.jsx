...
import BurroVentaForm from './BurroVentaForm';

const BurrosVentaInventory = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const { error } = await supabase.from('burros_venta').delete().eq('id', id);
      if (error) throw error;
        ...
        <table className="w-full text-sm text-left">
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              ...
              <tr>
                <td colSpan="6" className="px-4 py-8 text-center text-gray-500">No se encontraron registros.</td>
              </tr>
            ) : (
              filteredData.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-gray-900 font-semibold">
                    {item.precio ? new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP' }).format(item.precio) : '-'}
                  </td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                      item.estado === 'Disponible' ? 'bg-green-100 text-green-800' : 
                      item.estado === 'En negociación' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'
                    }`}>
...
export default BurrosVentaInventory;