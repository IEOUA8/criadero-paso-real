...
import VientrForm from './VientrForm';

const VientresInventory = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const { error } = await supabase.from('vientres').delete().eq('id', id);
      if (error) throw error;
    ...
    <div className="p-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        ...
        <Button onClick={() => openModal()} className="w-full sm:w-auto bg-[#C8A94B] hover:bg-[#C8A94B]/90 text-[#0B0B0B] font-bold shadow-md">
          <Plus className="w-4 h-4 mr-2" /> Nuevo Vientre
        </Button>
      </div>
                ...
                <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                      ...
                      item.estado === 'En negociación' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800'
                    }`}>
...
export default VientresInventory;